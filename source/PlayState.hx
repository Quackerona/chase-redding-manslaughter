...
class PlayState extends MusicBeatState
{
	...
	// cahse reding
	public var waterfall:BGSprite;
	public var greenhill:BGSprite;
	...
	override public function create()
	{
		...
		{
			case "2":
				waterfall = new BGSprite("NE/NEgreenhills", 0, 0, 1, 1, ["NEgreenhills loop"], true);
				waterfall.scale.set(0, 0); //scaling to 0 instead of setting visibility so it caches
				add(waterfall);
				
				greenhill = new BGSprite("NE/NEbg2", 0, 0, 1, 1, ["NEbg2 NEbg2"], true);
				greenhill.scale.set(0, 0);
				add(greenhill);
			case 'stage': //Week 1
		...
		DiscordClient.changePresence(detailsText, SONG.song + " (" + storyDifficultyText + ")", iconP2.getCharacter(), "game");
		#end
		...
		FlxG.camera.setFilters([new ShaderFilter(ShaderObject.shaderHandler.crt), new ShaderFilter(ShaderObject.shaderHandler.distortion)]);

		eventHandler = new Events(Paths.formatToSongPath(SONG.song));
	...
	function startSong():Void
	{
		...
		DiscordClient.changePresence(detailsText, SONG.song + " (" + storyDifficultyText + ")", iconP2.getCharacter(), "game", true, songLength);
		#end
	...
	override function closeSubState()
	{
		if (paused)
		{
			...
			if (startTimer != null && startTimer.finished)
			{
				DiscordClient.changePresence(detailsText, SONG.song + " (" + storyDifficultyText + ")", iconP2.getCharacter(), "game", true, songLength - Conductor.songPosition - ClientPrefs.noteOffset);
			}
			else
			{
				DiscordClient.changePresence(detailsText, SONG.song + " (" + storyDifficultyText + ")", iconP2.getCharacter(), "game");
	...
	override public function onFocusLost():Void
	{
		...
		if (health > 0 && !paused)
		{
			DiscordClient.changePresence(detailsPausedText, SONG.song + " (" + storyDifficultyText + ")", iconP2.getCharacter(), "game");
		}
		#end
	...
	function openChartEditor()
	{
		...
		#if desktop
		DiscordClient.changePresence("Chart Editor", null, null, "game",true);
		#end
	}
	...
	function doDeathCheck(?skipHealthCheck:Bool = false) {
		if (((skipHealthCheck && instakillOnMiss) || health <= 0) && !practiceMode && !isDead)
		{
			...
			if(ret != FunkinLua.Function_Stop) {
				...
				DiscordClient.changePresence("Game Over - " + detailsText, SONG.song + " (" + storyDifficultyText + ")", iconP2.getCharacter(), "game");
				#end
				isDead = true;
				return true;
			}
	...
	{
		super.stepHit();
		eventHandler.onStep(curStep); //“I’M GONNA KISS A GUY” - eenat
		                              //"2" - case reding
